...
cv2.createTrackbar("L - S", "Trackbars", 0, 255, nothing)  # 낮은 Saturation 값 설정
cv2.createTrackbar("L - V", "Trackbars", 200, 255, nothing)  # 낮은 Value 값 설정
cv2.createTrackbar("U - H", "Trackbars", 255, 255, nothing)  # 높은 Hue 값 설정
cv2.createTrackbar("U - S", "Trackbars", 50, 255, nothing)  # 높은 Saturation 값 설정
...
def image_callback(msg):
    try:
        ...
        circles = cv2.HoughCircles(
            ...
            dp=1,                     # 해상도 비율
            minDist=200,               # 원의 중심들 간의 최소 거리 설정
        ...
        if circles is not None:
            ...
            for (x, y, r) in circles:
                if 10 <= r <= 100:  # 반지름 크기를 필터링
                    # 원이 이미지 경계를 벗어나지 않도록 조건을 추가
                    if y - r >= 0 and y + r < mask.shape[0] and x - r >= 0 and x + r < mask.shape[1]:
                        circle_roi = mask[y-r:y+r, x-r:x+r]  # 원의 ROI (Region of Interest) 설정
                        if circle_roi.size > 0:  # ROI가 유효한지 확인
                            edges = cv2.Canny(circle_roi, 50, 150)  # Canny 에지 검출기 적용
